...
#define LCD_DC_PIN            GET_PIN(D, 12)
#define LCD_CS_PIN            GET_PIN(D, 7)  // 新增CS引脚控制
...
static int rt_hw_lcd_config(void)
{
    ...
    cfg.max_hz = 21 * 1000 * 1000; /* 降低为21MHz以匹配商家配置 */
...
{
    rt_pin_write(LCD_CS_PIN, PIN_LOW);  // 新增CS控制
    rt_pin_write(LCD_DC_PIN, PIN_LOW);

    rt_size_t len = rt_spi_send(spi_dev_lcd, &cmd, 1);

    rt_pin_write(LCD_CS_PIN, PIN_HIGH); // 结束传输

    return len == 1 ? RT_EOK : -RT_ERROR;
}
...
static rt_err_t lcd_write_data(const rt_uint8_t data)
{
    ...
    rt_size_t len = rt_spi_send(spi_dev_lcd, &data, 1);
...
static rt_err_t lcd_write_half_word(const rt_uint16_t da)
{
    ...
    char data[2] = {da >> 8, da};
    rt_size_t len = rt_spi_send(spi_dev_lcd, data, 2);

    rt_pin_write(LCD_CS_PIN, PIN_HIGH); // 结束传输

    return len == 2 ? RT_EOK : -RT_ERROR;
}
...
static void lcd_gpio_init(void)
{
    ...
    rt_pin_mode(LCD_CS_PIN, PIN_MODE_OUTPUT);  // 初始化CS引脚
    rt_pin_mode(LCD_PWR_PIN, PIN_MODE_OUTPUT);

    rt_pin_write(LCD_CS_PIN, PIN_HIGH);  // 默认CS高电平
    rt_pin_write(LCD_PWR_PIN, PIN_LOW);

    // 硬件复位被禁用（RES接地），通过延时确保电源稳定
    rt_thread_mdelay(120);  // 延长到120ms以匹配商家代码
}

static int rt_hw_lcd_init(void)
{
    __HAL_RCC_GPIOD_CLK_ENABLE();
    rt_hw_spi_device_attach("spi3", "spi30", GPIOD, GPIO_PIN_11);
    lcd_gpio_init();
    ...
    /* 修改为16位像素格式 */
    lcd_write_cmd(0x3A);
    lcd_write_data(0x05);  // 改为0x05表示16位色
